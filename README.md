...
- Авиабилеты: каталог рейсов (управляет MANAGER/ADMIN) + заявки клиентов + чат по заявке.
...
- Создает заявку на авиабилет (выбор направлений/дат/пассажиров), после чего ведет диалог с менеджером в чате.
...
- Управляет рейсами: создает/редактирует, архивирует/восстанавливает, видит заявки на авиабилеты и меняет их статусы.
...
- Авиабилеты:
  - Рейсы и заявки: publicId для отображения, UUID для чата/внутренних связей (`src/types/api.ts`).
  - Архив рейсов: публичный API отдает только активные, для админки используются `/api/charter/flights/admin*`.
  - UX флоу "заполнил на публичной части -> авторизация -> создание заявки": через `/flights/continue`.

Примечание по неймингу: в коде и API домен называется `charter`, в UI/маршрутах для пользователей используется терминология "авиабилеты" и пути `/client/flights`, `/manager/flights`.
...
- Дальнейшее развитие авиабилетов: расширенные фильтры/поиск, улучшение UX выбора дат/расписаний, уведомления по статусам.
- Интеграция с amoCRM.
